...
func (p *Program) TokenLiteral() string {
	if len(p.Statements) > 0 {
		return p.Statements[0].TokenLiteral() // TokenLiteral is a helper function
	} else {
		return ""
	}
}
...
type ReturnStatement struct {
	...
	ReturnValue Expression  // ReturnValue is the expression to be returned
}
...
type PrefixExpression struct {
	...
	Right    Expression  // The right expression, e.g. 5
}
...
type InfixExpression struct {
	...
	Left     Expression  // The left expression, e.g. 5
	Operator string      // The operator, e.g. +
	Right    Expression  // The right expression, e.g. 5
}
...
type IndexExpression struct {
	...
	Index Expression  // The index expression, e.g. 1
}
